...
public class TextPreprocessing {
    ...
    public static void main(String[] args) {
        ...
        String COMMA = ",";
        try {
            FileWriter fileWriter = new FileWriter("listFathin.csv");
            
            fileWriter.append("No");
            for (int i = 0; i < frekuensi.length; i++) {
                if (frekuensi[i] != isVisited){
                    num++;
                    fileWriter.append(listKataSemua.get(i));
            ...
            for (int i = 0; i < frekuensi.length; i++) {
                if (frekuensi[i] != isVisited){
                    num++;
                    fileWriter.append(String.valueOf(frekuensi[i]));
                    fileWriter.append(COMMA);
                }
            }
            ...
            for (int u = 0; u < 10; u++) {
                for (int i = 0; i < listOfFiles.length; i++) {
                    if (listOfFiles[i].isFile()) {
                        String fileName = listOfFiles[i].getName();
                        if (fileName.startsWith("a_positif")) {
                            try {
                                File f = new File(fileName);
                                reader = new BufferedReader(new FileReader(f));
